...
CodeRegionManager* CodeRegionManager::get_instance() {
    static CodeRegionManager instance;
    return &instance;
}

CodeRegionManager::CodeRegionManager() {
    regions_.store(new std::vector<CodeRegion>(), std::memory_order_release);
}

CodeRegionManager::~CodeRegionManager() {
    delete regions_.load(std::memory_order_acquire);
}

bool CodeRegionManager::contains(uintptr_t addr) const {
    return find(addr) != nullptr;
}

const CodeRegion* CodeRegionManager::find(uintptr_t addr) const {
    // RCU 读路径: 无锁，直接读取原子指针
    auto* regions = regions_.load(std::memory_order_acquire);
    if (!regions || regions->empty()) return nullptr;

    // 二分查找 O(log n)
    int left = 0, right = (int)regions->size() - 1;
    while (left <= right) {
        int mid = left + (right - left) / 2;
        const auto& r = (*regions)[mid];
...
void CodeRegionManager::add_region(uintptr_t start, uintptr_t end, RegionType type, const std::string& name) {
    std::lock_guard<std::mutex> lock(write_mutex_);

    // 复制当前列表
    auto* old_regions = regions_.load(std::memory_order_acquire);
    auto* new_regions = new std::vector<CodeRegion>(*old_regions);

    // 检查是否已存在
    for (auto it = new_regions->begin(); it != new_regions->end(); ++it) {
        if (it->start == start) {
            it->end = end;
            it->type = type;
            it->name = name;
    ...
    // 添加新区域并排序
    new_regions->push_back({start, end, type, name});
...
void CodeRegionManager::remove_region(uintptr_t start) {
    ...
    auto* old_regions = regions_.load(std::memory_order_acquire);
    auto* new_regions = new std::vector<CodeRegion>();

    for (const auto& r : *old_regions) {
        if (r.start != start) {
...
std::vector<CodeRegion> CodeRegionManager::get_regions() const {
    auto* regions = regions_.load(std::memory_order_acquire);
    return regions ? *regions : std::vector<CodeRegion>();
}
...
GumTrace::GumTrace() {
    ...
    region_manager = CodeRegionManager::get_instance();
}
...
#        else
    ...
    for (int i = 0; i < callback_ctx->instruction_detail.arm64.op_count; i++) {
        ...
        if ((op.access & CS_AC_READ) && (op.access & CS_AC_WRITE) && op.type == ARM64_OP_REG) {
            ...
            is_write = true;
            self->write_reg_list.add(op.reg);  // 修复: 使用带边界检查的 add()
            ...
            if (strstr(callback_ctx->instruction.op_str, "],") || strstr(callback_ctx->instruction.op_str, "]!")) {
                is_write = true;
                self->write_reg_list.add(op.mem.base);  // 修复: 边界检查
            }
            ...
            is_write = true;
            self->write_reg_list.add(op.reg);  // 修复: 边界检查
        }
    }
...
void GumTrace::transform_callback(GumStalkerIterator *iterator, GumStalkerOutput *output, gpointer user_data) {
    ...
    while (gum_stalker_iterator_next(it, (const cs_insn **) &p_insn)) {
        ...
        if (module_name_ptr == nullptr) {
            // 模块未命中，检查 JIT 区域 (O(log n) 二分查找)
            const CodeRegion* region = self->region_manager->find(addr);
            if (region == nullptr) {
                // 既不在模块内也不在 JIT 区域内，跳过
                gum_stalker_iterator_keep(it);
                continue;
            }
            // 命中 JIT 区域，使用 JIT 区域名作为模块名
            ...
            if (gum_stalker_iterator_get_memory_access(it) != GUM_MEMORY_ACCESS_EXCLUSIVE) {
                ...
                auto callback_ctx = self->callback_context_instance->pull(
                    p_insn, module_name_ptr->c_str(), module.at("base"));
                gum_stalker_iterator_put_callout(it, callout_callback, callback_ctx, nullptr);
            }
        }
...
bool GumTrace::in_trace_range(uintptr_t address) {
    // 检查已知模块
    if (in_range_module(address) != nullptr) return true;
    // 检查 JIT 区域
    return region_manager->contains(address);
}
